...
const { ArgumentError, KrunkerError } = require('../errors/index.js');
...
const getPreview = t => `https://assets.krunker.io/textures/${
    t.type === 4
        ? 'sprays/' + t.id
        : `previews/${
            (
                t.type && (t.type < 3 || t.type > 4)
                    ? `cosmetics/${t.type}_${t.id + (t.tex ? '_' + t.tex : '')}`
                    : types[t.type || 0] + (
                        t.type === 3
                            ? t.id + (!t.pat ? !t.tex ? '' : '_' + t.tex : '_c' + t.pat)
                            : `${t.weapon || 0}_${(
                                !t.mid
                                    ? !t.pat
                                        ? t.tex || t.id
                                        : 'c' + t.pat
                                    : 'm' + t.mid + (!t.midT ? '' : '_' + t.midT))
                            }.png?build=u9K3c`
                    )
            )}`
}`;
const getTexture = i => {
    ...
    // if (texture === 'https://assets.krunker.io/textures/.png' || texture.indexOf('undefined') > 0) throw new KrunkerError('NO_SKIN_DATA');
    return { e: emissive, t: texture };
};
const toHexRGB = ole =>
    ole < 0 || ole > 16777215 ? '#000000' : '#' + [ole % 256, (ole / 256) % 256, (ole / 65536) % 256]
        .map(x =>
            Math
                .floor(x)
                .toString(16)
...
module.exports = class Skin {
    constructor(client, wResolvable, data) {
        ...
        this.textureImage = t;
        this.textureEmissive = this.glow ? e : null;
